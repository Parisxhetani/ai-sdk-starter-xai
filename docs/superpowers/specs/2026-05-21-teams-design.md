...
- **Global chat author visibility:** with `users` SELECT scoped to same-team rows, a user joining `messages.user_id → users` in global chat would not be able to read another team's author name. Resolution: add a narrower policy to `users` that allows SELECT of `id, name` (a minimal subset) for *any* authenticated user, while keeping email/phone/role/team_id same-team-only. Implementation handles this via a separate read policy on a restricted column set or a dedicated `public_user_names` view.
