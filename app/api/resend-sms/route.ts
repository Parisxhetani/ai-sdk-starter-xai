...
export async function POST(request: NextRequest) {
  try {
    ...
    const smsMessage = `[RESEND] Friday order – Tony's (Tirana): ${orders.length} meals. ${summaryText}. Contact: ${adminPhone}.`
    ...
    if (process.env.TWILIO_ACCOUNT_SID && process.env.TWILIO_AUTH_TOKEN && process.env.TWILIO_PHONE_NUMBER) {
      try {
        ...
        smsResult = { sid: "mock_resend_fallback_" + Date.now(), status: "sent" }
      }
    } else {
      // Mock SMS for demo
      console.log("[v0] Mock SMS resent to:", tonyPhone)
      console.log("[v0] Message:", smsMessage)
      smsResult = { sid: "mock_resend_" + Date.now(), status: "sent" }
    }

    // Log SMS resent event
    ...
    console.error("SMS resending error:", error)
    return NextResponse.json({ error: "Failed to resend SMS" }, { status: 500 })
  }
}
